...
const resize = () => {
  ...
  // Prefer viewport size to avoid any parent sizing quirks
  const vw = Math.max(window.innerWidth || 0, document.documentElement?.clientWidth || 0);
  const vh = Math.max(window.innerHeight || 0, document.documentElement?.clientHeight || 0);

  const rw = Math.max(1, Math.floor(vw * (props.resolutionScale || 1)));
  const rh = Math.max(1, Math.floor(vh * (props.resolutionScale || 1)));

  renderer.setSize(rw, rh);
  // Use render target resolution for shader math to avoid edge artifacts
